...
public class MatriculacionServicio {
	...
	/**
	 * Envía una solicitud PUT a la API para modificar una matrícula existente.
	 *
	 * @param idMatriculacion ID de la matrícula a modificar.
	 * @param dto             DTO con los nuevos datos de la matrícula.
	 * @return true si la matrícula se modificó correctamente, false si no.
	 */
	public boolean modificarMatriculacion(Long idMatriculacion, MatriculacionDto dto) {
		try {
			Gson gson = new Gson();
			String json = gson.toJson(dto);

			// URL de la API (asegúrate de que sea la correcta)
			String urlApi = "http://localhost:9527/api/modificarMatriculacion/" + idMatriculacion;
			...
			// Enviar el JSON al body de la petición
			try (OutputStream os = conn.getOutputStream()) {
				os.write(json.getBytes("UTF-8"));
			}

			int responseCode = conn.getResponseCode();
			Log.ficheroLog("Respuesta de la API PUT /modificarMatriculacion: " + responseCode);

			return responseCode == HttpURLConnection.HTTP_OK;

		} catch (Exception e) {
			Log.ficheroLog("Error en servicio modificarMatriculacion: " + e.getMessage());
			e.printStackTrace();
			return false;
		}
	}
